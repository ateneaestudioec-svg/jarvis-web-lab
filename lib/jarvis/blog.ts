...
async function getDraftId(slug: string) {
  const safeSlug = normalizeSlug(slug);
  if (!safeSlug) throw new Error("Invalid slug.");
  const id = await getBlogClient().fetch<string | null>(
    `*[_type == "blogPost" && _id in path("drafts.**") && slug.current == $slug][0]._id`,
    { slug: safeSlug },
    { cache: "no-store", perspective: "raw" },
  );
  if (!id) throw new Error("Draft was not found.");
...
export async function updateBlogDraft(slug: string, changes: BlogUpdate) {
  const { id } = await getDraftId(slug);
  ...
  revalidateBlog();
  return getBlogPost((patch.slug as { current?: string } | undefined)?.current ?? slug);
}
