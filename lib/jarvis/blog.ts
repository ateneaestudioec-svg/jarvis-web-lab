...
export async function publishBlogPost(slug: string) {
  ...
  const draft = await client.fetch<Record<string, unknown> | null>(
    `*[_type == "blogPost" && status == "draft" && slug.current == $slug][0]`,
    { slug: safeSlug },
    { cache: "no-store", perspective: "raw" },
  );
  ...
  // Publish through a normal Sanity mutation instead of the Actions endpoint.
  // Only copy fields owned by the blog schema: system metadata from the draft
  // must never be sent when creating the published document.
  ...
  for (const field of ["excerpt", "mainImage", "seoTitle", "seoDescription"] as const) {
    if (draft[field] !== undefined && draft[field] !== null) {
      publishedDocument[field] = draft[field];
    }
  }

  // Persist the public document first. Draft cleanup is deliberately separate:
  // a cleanup failure must not roll back an otherwise valid publication.
  await client.createOrReplace(publishedDocument, { visibility: "sync" });

  const draftId = String(draft._id);
  if (draftId !== publishedId) {
    try {
      await client.delete(draftId, { visibility: "sync" });
    } catch (error) {
      console.warn(
        "The article was published, but its draft could not be cleaned up.",
