...
export async function getBlogPost(slug: string): Promise<BlogPostContent | null> {
  ...
  return getBlogClient().fetch<BlogPostContent | null>(
    `*[_type == "blogPost" && slug.current == $slug] | order(_id in path("drafts.**") desc)[0]{
      title,
      "slug": slug.current,
      excerpt,
      seoTitle,
...
async function getEditableId(slug: string) {
  ...
  const id = await getBlogClient().fetch<string | null>(
    `*[_type == "blogPost" && slug.current == $slug] | order(_id in path("drafts.**") desc)[0]._id`,
    { slug: safeSlug },
    { cache: "no-store", perspective: "raw" },
  );
...
export async function publishBlogPost(slug: string) {
  ...
  const { _rev: _revision, _createdAt: _created, _updatedAt: _updated, ...draftContent } = draft;
  void _revision;
  void _created;
  void _updated;
  const publishedDocument: { _id: string; _type: string; [key: string]: unknown } = {
    ...draftContent,
    _id: publishedId,
    _type: "blogPost",
    status: "published",
  ...
  await client.transaction().createOrReplace(publishedDocument).delete(String(draft._id)).commit();
