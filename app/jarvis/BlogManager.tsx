...
export default function BlogManager({ isJarvisLoading, refreshSignal, onAskJarvis }: Props) {
  ...
  async function publish() {
    if (!draft || isNew || draft.status === "published" || isPublishing || hasChanges) return;
    setIsPublishing(true);
    setError(null);
    try {
      const formData = new FormData();
      formData.set("mode", "publish");
      formData.set("slug", draft.slug);
      formData.set("confirmation", `publish:${draft.slug}`);
      const response = await fetch("/api/jarvis/blog", { method: "POST", body: formData });
      const data = await parseResponse<{ post: BlogPost; posts: BlogSummary[] }>(response);
      setPosts(data.posts);
      setOriginal(data.post);
      setDraft(data.post);
      setNotice("✓ Artículo publicado");
      setConfirmPublish(false);
    } catch (publishError) {
      setError(publishError instanceof Error ? publishError.message : "No se pudo publicar la entrada.");
    } finally {
    ...
    <div className={styles.blogManager}>
      ...
      <div className={styles.blogLayout}>
        <aside className={styles.postList} aria-label="Entradas de Blog">
          ...
          {posts.map((post) => (
            <button type="button" key={`${post.status}-${post.slug}`} className={draft?.slug === post.slug && !isNew ? styles.activePost : ""} onClick={() => selectPost(post.slug)}>
          ...
          {isLoading && !draft ? <p className={styles.loading}>Cargando entradas...</p> : draft ? (
            <>
              <div className={styles.blogEditorHeader}>
                <span>{isNew ? "Nuevo borrador" : draft.status === "published" ? "Editando publicación" : "Editando borrador"}</span>
              ...
              {!isNew && draft.status === "draft" && <button className={styles.publishButton} type="button" disabled={hasChanges || isSaving} onClick={() => { setError(null); setConfirmPublish(true); }}>Publicar</button>}
              {!isNew && draft.status === "published" && <a className={styles.viewArticle} href={`/blog/${draft.slug}`} target="_blank" rel="noopener noreferrer">Ver artículo ↗</a>}
            </>
          ) : <p className={styles.loading}>Selecciona una entrada o crea un borrador.</p>}
        </div>
      </div>
      {confirmPublish && draft && <div className={styles.modalBackdrop} role="presentation">
        <div className={styles.confirmModal} role="dialog" aria-modal="true" aria-labelledby="publish-title">
          <p>Publicación</p>
          <h2 id="publish-title">¿Publicar esta entrada?</h2>
          <span>El artículo será visible públicamente en /blog.</span>
          <div><button type="button" onClick={() => setConfirmPublish(false)} disabled={isPublishing}>Cancelar</button><button type="button" onClick={() => void publish()} disabled={isPublishing}>{isPublishing ? "Publicando..." : "Publicar"}</button></div>
        </div>
      </div>}
    </div>
  );
