...
export default function JarvisPage() {
  ...
  const [blogRefreshSignal, setBlogRefreshSignal] = useState(0);
    ...
    try {
      ...
      if (data.contentChanged) {
        try {
          ...
          setSaveState("idle");
          selectManualImage(null);
          setBlogRefreshSignal((current) => current + 1);
        } catch (syncError) {
    ...
    <main className={styles.page}>
      ...
      <div className={styles.workspace}>
        <section className={styles.contentPanel} aria-labelledby="content-title">
          <nav className={styles.contentNav} aria-label="Secciones de contenido">
            <button type="button" className={activeSection === "home" ? styles.activeNav : ""} onClick={() => setActiveSection("home")}>Home</button>
            <button type="button" className={activeSection === "blog" ? styles.activeNav : ""} onClick={() => setActiveSection("blog")}>Blog</button>
          </nav>
          {activeSection === "home" ? (
          <>
          ...
          {isContentLoading || !draft ? (
            ...
            <SectionsManager refreshSignal={blogRefreshSignal} />
            </>
          )}
          </>
          ) : (
            <BlogManager
              isJarvisLoading={isLoading}
              refreshSignal={blogRefreshSignal}
              onAskJarvis={(prompt) => void sendInstruction(prompt)}
            />
          )}
        </section>
